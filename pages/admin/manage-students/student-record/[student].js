...
const StudentRecord = ({
    ...
    const loadStudent = async () => {
        try {
            const { data } = await axios.get(`/api/admin/student/${student}`);
    ...
    const openModal = (item) => {
        ...
        router.push(`/admin/manage-activities/quiz/${slug}`)
      } else if (item.itemType === 'Assignment') {
        router.push(`/admin/manage-activities/assignment/${slug}`)
      } else {
        router.push(`/admin/manage-activities/interactive/${slug}`)
      }
    }
    return (
        <AdminRoute>
