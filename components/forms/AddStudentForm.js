...
const AddStudentForm = ({ students, courseId }) => {
    const data = [];
    ...
    const onRemoveStudent = async (student) => {
        ...
        try {
            const { removeStudent } = await axios.put(`/api/admin/course/${courseId}/remove-student/${student.key}`)
        } catch (error) {
            console.log(error)
        }
    }
    ...
    const onAddStudent = async (student) => {
        ...
        // sending data to backend
        try {
            const { addStudent } = await axios.put(`/api/admin/course/${courseId}/add-student/${student.key}`)
        } catch (error) {
            console.log(error)
        }
    }
